...
If you generate your own assembly, we strongly recommend that your contigs have a consistent naming format. The format we assume for this workflow is `NIOZ114_sc105_1`. Ideally have a short and concice header that includes the sampleID and no other symbols than underscores.

<span style="color:red;font-size:20px">**Notice for this workshop:**</span>

Unfortunately, DAS Tool is not installed on the cluster. As a workaround we will provide two options:

1. All the output data is provided in the zenodo repository found [here](https://zenodo.org/record/6489670#.YnoBgelBxcA). Specifically, you can download the folder Example_Data.tar.gz and have a look at how the output from DAS Tool looks like in the folder 3_DASTool and use the files in 4_FinalBins to finish with the last small steps summarizing the genome statistics.
2. You can download the tool itself [here](https://github.com/cmks/DAS_Tool) and the installation instruction can be found further down in the documentation when you follow the link. From the test I ran on my Mac, the conda install did not work, but the brew and docker installs look good. If you use the docker setup you might just need to run the program 3 times for each of the 3 assemblies instead of using the for-loop.
...
First lets change our directory to where we work. Exchange the path to where you downloaded the data.The files are available on Zenodo [here](https://zenodo.org/record/4680187#.YHQRUI1R03g). Have patience, they take a while to download.
...
wget https://raw.githubusercontent.com/ndombrowski/Binning_tutorial/master/scripts/rename.pl -O scripts/rename.pl
```

Now, lets prepare a working directory and go into it and work from there:
...
Save and close nano (by pressing control + X).
...
awk -F'\t' -v OFS='\t' '{ a[$0] }END {for (i in a){for (j in a){ print (i, j)}}}' FileLists/Samples | sort > FileLists/Samples_matching
```

The file we generated should look like this (tab separated). 
...
The mapping step takes a lot of time and the data from the read mapping is rather large. <br> Therefore, you will not run the read mapping step with `bwa mem` yourself, but use the data you have downloaded from the repository in the first steps. After the tutorial feel free to run the code under #create 1_mapping files but watch the resources on your cluster before doing so. <br/><br/>

More specifically, you only need to run code that does NOT start with an `#` and the assembly and read mapping data is provided by us. <br/><br/>

If you want to have a look at the data, they can be found in this folder: `../0_assembly_and_reads/2_assembly/`. <br/><br/>
</div>
...
For each metagenome the step `bwa mem`, would generate a folder, each containing three files:
...
We provide you the indexed samfiles that are normally created with the step `#create bam files and make index` in the folder you downloaded. To access them we generate a symbolic link via the `ln` command. A symbolic link is a special type of file that points to another file or directory, without us having to duplicate data.
...
`r text_spec("The code below is some extra code for showing you how the coverage for one indivdual contig looks like. You do not need to run this right now, but feel free to test it later ", background = "#D05A6E", color = "white", bold = T)`
...
`r text_spec("Important: Since we do not work with BinSanity for this tutorial, do not run this step!", background = "#D05A6E", color = "white", bold = T)`
...
**A small comment on pipes in bash:**
A pipe is a form of redirection (transfer of standard output to some other destination). The code block below is doing the same as in `for i in `cat FileLists/Samples`; do ll 2_binning/${i}/Metabat/bins/*fa | wc -l; done | paste FileLists/Samples -` but in astep by step manner.
...
for i in `cat FileLists/Samples`; do ll 2_binning/${i}/Metabat/bins/*fa > ${i}_temp; done
...
The program `rename` generally is installed on all unix platforms but can work slightly differently depending on its source.  <br/><br/>

The format rename old_name new_dame file works for util-linux 2.23.2  <br/>
The format rename s/old_name/new_name/g file works for perl distributions (often found on macs) <br/><br/>

If the command below is not working, check what version of rename you have. If you do not get it to work ignore the renaming steps, the code below still should work. <br/><br/>
...
`r text_spec("We will NOT run Concoct to keep the tutorial short but if you run this on your own data it is advisable to use several binning tools and concoct would be an example of a binner to use.", background = "#D05A6E", color = "white", bold = T)`
...
Specifically, we want to remove unwanted symbols, such as dots, and we want the file names to have the extension .fa instead of .fasta.
...
When we check the bin names, we can see that the bins come from both binning tools, if we would use more binning tools, i.e. include concoct and binsanity, we would see how hard it would be to decide which single tool would be best.
...
#cleanup filenames: adjust the extension, remove the contig info and add the sample ID into the file header
...
<div class="alert-danger">
Unfortunately, CheckM is not installed on the severs. However, we still list all relevant code since it is a key program to check bin quality. Install instructions can be found [here](https://github.com/Ecogenomics/CheckM/wiki/Installation#how-to-install-checkm) <br/><br/>
...
knitr::include_graphics("/Users/ninadombrowski/Desktop/WorkingDir/Notebooks/Pictures/Annotations17.png")
```
...
`r text_spec("For this tutorial we will NOT run this step. However, it is a good tool to keep in mind if you ever do this analysis on your own data", background = "#D05A6E", color = "white", bold = T)`
...
```{r, echo=FALSE, fig.cap="", out.width = '100%', fig.align="left"}
knitr::include_graphics("/Users/ninadombrowski/Desktop/WorkingDir/Notebooks/Pictures/Annotations18.png")
```
...
[We check this with:<br/><br/>grep -c ">" ../0_assembly_and_reads/2_assembly/AllContigs/All_contigs_2500.fna<br/>wc -l 7_Stats/Length_Bin_length_final.txt<br/><br/>Initially we have 1028 contigs, of these 874 contigs were binned]{.spoiler}
...
`r text_spec("As this relies on the gtdb_tk step, which we have not done, don't do this.", background = "#D05A6E", color = "white", bold = T)`
